...
impl ServerHandler {
    pub fn start_server<A, F, H>(address: A, handler: F) -> ServerResult<Self>
        where A: ToSocketAddrs + Send + 'static,
              F: Fn() -> H + Send + Clone + 'static,
              H: IntoHttpHandler + 'static {
        let (sender, receiver) = sync_channel::<ServerResult<ServerHandler>>(0);
        thread::spawn(move || {
...
fn start_server_curr_actix_system<F, H>(address: impl ToSocketAddrs, handler: F) -> ServerResult<ServerHandler>
